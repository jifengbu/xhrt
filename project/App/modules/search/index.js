...
var SearchAll = require('./SearchAll.js');
var SearchVideoList = require('./SearchVideoList.js');
var SearchQuestionList = require('./SearchQuestionList.js');
var SearchRoomList = require('./SearchRoomList.js');
...
module.exports = React.createClass({
    ...
    doSearchSuccess(keyword, data) {
        if (data.success) {
            var {videoList, questionList, roomList} = data.context;
            videoList = videoList||[], questionList = questionList||[], roomList = roomList||[];
    ...
    doSearchVideoSuccess(keyword, data) {
        if (data.success) {
            var videoList = data.context.videoList||[];
            if (!videoList.length) {
                Toast("没有相关视频");
                return;
            }
    ...
    doSearchQuestionSuccess(keyword, data) {
        if (data.success) {
            var questionList = data.context.questionList||[];
    ...
    doSearchRoomSuccess(keyword, data) {
        if (data.success) {
            var roomList = data.context.roomList||[];
            if (!roomList.length) {
                Toast("没有相关房间");
                return;
            }
