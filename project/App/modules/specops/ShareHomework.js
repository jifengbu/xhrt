...
const EditInformationBox = require('../person/EditInformationBox.js');
...
module.exports = React.createClass({
    ...
    componentDidMount () {
        const { taskContent, taskID, userTaskID, videoID, taskName } = this.props.data;
        this.textID = videoID;
        const textContent = InputTextMgr.getTextContent(this.textID);
        if (taskContent && taskContent != '') {
            ...
            Toast('继续提交上次内容');
            this.setState({ inputText: textContent, isFirstTap: false ,isCache: true});
        }
        if (taskID) {
    ...
    doConfirm () {
        ...
        const { taskContent } = this.props.data;
        if (!taskContent && !this.state.isSummit) {
            this.setState({ isShare: false, isFirstTap: true, inputText: '' ,isCache: false});
    ...
    toSave () {
        const { inputText, taskID } = this.state;
        // 提交数据
        if (!inputText) {
            Toast('提交失败，请填写内容');
            return;
        ...
        POST(app.route.ROUTE_SUBMIT_SPECIAL_SOLDIER_TASK, param, this.submitSpecialSoldierTaskSuccess, this.submitSpecialSoldierTaskError, true);
    },
    submitSpecialSoldierTaskSuccess (data) {
        if (data.success) {
            Toast('提交作业成功');
            this.setState({ userTaskID:data.context.specopsUserTask.id, isShare:true, isSummit: true ,isCache:false});
            app.getCurrentRoute().leftButton = { handler: () => { app.scene.doBack(); } };
            app.getCurrentRoute().rightButton = { };
            app.forceUpdateNavbar();
            InputTextMgr.removeItem(this.textID);
    ...
    doShare (item) {
      // 0,1,2分别表示微信，朋友圈，QQ
        const { inputText, taskID, userTaskID } = this.state;
        if (!inputText) {
            Toast('内容为空');
            return;
        }
        ...
        const data = 'homeworkID=' + userTaskID;
        const desc = '特种兵：' + info.name + ' /' + inputText;
    ...
    render () {
        const { isShare, isKeyboard, heightIntroduce ,isCache} = this.state;
        const titleHeight = this.calculateStrLength(this.state.taskName, 42);
        const comHeight = this.calculateStrLength(this.state.inputText, 42);
        let textHeight = 0;
        let isHeight = true;
        if (isShare) {
                    ...
                    <View style={!isShare && isKeyboard ? styles.midEditView : styles.midView}>
                        {
                            !isShare && isKeyboard ?
                                ...
                                /> :
                                <Text onPress={isShare ? this.onFocus : isCache?this.exitHomework:this.changKeyboard} style={[styles.textView, isHeight ? { height: textHeight } : null]}>{this.state.inputText}</Text>
