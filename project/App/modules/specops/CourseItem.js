...
module.exports = React.createClass({
    ...
    getSections(dataDetail) {
        ...
        if (dataDetail && dataDetail.otherVideo) {
            ...
            let sectionLength = Math.ceil(otherVideo.length/quarterlySeries);
    ...
    getVideoList(dataDetail, index) {
        ...
        if (dataDetail && dataDetail.otherVideo && this.props.dataArray) {
            ...
            let otherVideo = this.props.dataArray.slice();
            let len = otherVideo.length;
            let sectionLength = Math.ceil(len/quarterlySeries);
            let startNum = quarterlySeries * index;
            let endNum = Math.min(len, startNum+quarterlySeries);
            let row = 0, col = 0;
            for (var i = startNum; i < endNum; i++) {
                if (col === 0) {
                    list[row] = [];
                }
                var video = otherVideo[i];
