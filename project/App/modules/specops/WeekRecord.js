...
var WeekRecordItemView = require('./WeekRecordItemView.js');
...
module.exports = React.createClass({
    ...
    getScrollSize(index){
        var size = sr.rws(120);
        for (var i = 0; i < this.weekData.weekPlan.length; i++) {
            size += sr.rws(39);
        }
        for (var i = 0; i < this.weekData.dayPlan[index].length; i++) {
            size += sr.rws(39);
        }
        return size;
    },
    ...
    //每日总结
    weekPlanConclusion() {
        var {problemArray,daySummary} = this.state;
        let height = this.calculateStrLength(daySummary);
        return (
                ...
                </View>
                {
                    problemArray.map((item, i)=>{
                        if (item.problemContent) {
                            var textInputHeight = this.calculateStrLength(item.problemContent);
                        }
                        return(
                            <View key={i}>
