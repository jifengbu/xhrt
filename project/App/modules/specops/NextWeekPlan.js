...
module.exports = React.createClass({
    ...
    addDayPlanContent () {
        const textID = 'specops_nextWeekPlan1';
    ...
    doDeleteDayPlanSuccess (id, data) {
        if (data.success) {
            const dayData = this.getDayPlan(this.state.tabIndex);
            const planInfo = _.find(dayData, (item) => item.id == id);
            if (planInfo) {
    ...
    renderRowWeek (obj, sectionID, rowID) {
        const itemHeight = this.getItemHeight(obj.content);
            ...
            <View>
                <TouchableOpacity onPress={this.goWeekPlan} style={styles.titleContainerWeek}>
                    <View style={styles.titleContainerWeekSub}>
                        ...
                        <Image resizeMode='contain' source={app.img.specops_go_white} style={styles.goPlanImage} />
                    </View>
