...
module.exports = React.createClass({
    ...
    getReadingListSuccess (tabIndex, data) {
        if (data.success) {
            const articleList = data.context.articleList || [];
    ...
    changeTab (tabIndex) {
        const articleList = this.lists['articleList' + tabIndex];
    ...
    updateClickAndLikeNum (articleParam) {
        const articleList = this.lists['articleList' + this.state.tabIndex];
        const articleInfo = _.find(articleList, (item) => item.id == articleParam.articleId);
        if (articleInfo) {
    ...
    render () {
        const { tabIndex } = this.state;
        const menuAdminArray = ['名人文章', '学员分享', '赢销干货', '行业热点'];
        return (
