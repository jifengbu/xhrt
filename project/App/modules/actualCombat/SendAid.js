...
const { Button, DImage, ActionSheet, DelayTouchableOpacity } = COMPONENTS;
...
module.exports = React.createClass({
    ...
    doDeleteVoice () {
        ...
        if (this.tabIndex === 0) {
            ...
            const LocalArray = this.state.urlsingles;
            const netArray = this.state.urlsingle;
    ...
    uploadVoiceSuccessCallback (voiceTime, filePath, data) {
        if (data.success) {
            if (this.tabIndex === 0) {
                let array = this.state.urlsingle;
                array = array.concat(data.context.url);
                this.setState({ urlsingle: array });
                let LocalArray = this.state.urlsingles;
