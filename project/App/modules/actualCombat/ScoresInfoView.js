...
const { Button, PageList, RText } = COMPONENTS;

function getHalfStarNum (score) {
    const tempScore = Math.floor(score * 10);
    const subTempScore = tempScore % 10;
...
const ScoresView = React.createClass({
    statics: {
        title: '打分',
    },
    getInitialState () {
...
const AuthorView = React.createClass({
    render () {
        const retArray = getHalfStarNum(this.props.issueScore);
        const starNum = retArray.starNum;
        const starHalfNum = retArray.starHalfNum;
        let issueText;
...
const ScoresPanel = React.createClass({
    getInitialState () {
        return {
            starNum: 1,
            content: '',
...
const ScoresInfoPanel = React.createClass({
    getInitialState () {
        return {
            issueScore: 0,
    ...
    renderRow: function (rowData, sectionID, rowID) {
        const retArray = getHalfStarNum(rowData.score);
        const starNum = retArray.starNum;
        const starHalfNum = retArray.starHalfNum;
        let imageUrl;
...
module.exports = React.createClass({
    ...
    render () {
        ...
        const isSecond = this.state.tabIndex === 1;
        return (
