...
var AidDetail = require('./AidDetail.js');
...
module.exports = React.createClass({
    ...
    updateAidList(kitId, type) {
        ...
        this.listView.updateList((list)=>{
            var item = _.find(list, (o)=>o.id==kitId);
            if (item) {
                if (type === 0) {
                    item.persons += 1;
                } else {
    ...
    praiseKitsSuccess(rowID, data) {
        if (data.success) {
            var aidInfo = this.listView.list[rowID];
            if (aidInfo) {
    ...
    renderRow(obj, sectionID, rowID) {
        var isFirstTap = this.props.type===0;
        var tempPrice = obj.price.toFixed(2);
        return (
            <TouchableHighlight
