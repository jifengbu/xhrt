...
const sortByOrder = require('lodash.sortbyorder');
...
class Manager extends EventEmitter {
    ...
    onbroadcast (result) {
        ...
        const broadcastTypes = this.phone.broadcastTypes;
        let data;
        try {
    ...
    onNewGameReady (competitors) {
        const states = this.phone.channelStates;
        this.isInRoom = true;
        this.competitors = _.sortBy(competitors, (item) => item.seatNumber);
        const left = _.reject(this.competitors, (item) => item.channelState == states.MCAS_CHANNEL_STATE_EXIT || item.userID == app.personal.info.userID);
        if (!left.length) {
        ...
        this.round = 0;
        this.gradeList = {};
        const firstSpeaker = _.find(this.competitors, (item) => item.channelState == states.MCAS_CHANNEL_STATE_READY);
    ...
    onNextGameReady (competitors) {
        const states = this.phone.channelStates;
        _.forEach(this.competitors, (item) => {
            const person = _.find(competitors, (p) => p.userID == item.userID);
            if (!person) {
                this.status = STATUS_SERVER_INFO_ERROR;
            }
    ...
    onCompetitorsStateChange (userID, channelState) {
        const competitor = _.find(this.competitors, (item) => item.userID == userID);
        if (competitor) {
            const preState = competitor.channelState;
    ...
    dealWithCompetitorsExit (competitor, preState) {
        ...
        if (preState == states.MCAS_CHANNEL_STATE_SPEAKERING) {
            const speaker = _.find(this.competitors, (item) => item.channelState == states.MCAS_CHANNEL_STATE_READY);
            if (!speaker) {
            ...
            const seatNumber = competitor.seatNumber;
            const front = _.filter(this.competitors, (item) => item.seatNumber < seatNumber);
            let frontOver = true;
    ...
    dealWithCompetitorsGrade (userID, gradeUserID, gradeCount) {
        ...
        let competitorGrades = this.gradeList[gradeUserID];
        if (!competitorGrades) {
    ...
    checkEveryOneGrade (competitorGrades) {
        ...
        const left = _.reject(this.competitors, (item) => {
            return (!!_.find(competitorGrades, (p) => item.userID == p.userID)) || item.userID == this.gradeUserID;
        });
        const allGrade = _.every(left, (item) => item.channelState == states.MCAS_CHANNEL_STATE_EXIT);
        if (allGrade) {
    ...
    submitRoundScoreRank () {
        ...
        _.mapValues(this.gradeList, (list, userID) => {
            const scores = _.map(list, (item) => item.gradeCount);
            const n = scores.length || 1;
            const sum = _.sum(scores);
            const score = (parseInt(Math.round(sum / n * 10)) / 10) || 0;
        ...
        const rankList = _.map(scoreList, (item) => {
            const speaker = _.find(this.competitors, (p) => item.userID == p.userID);
    ...
    onJoinMeetingRoom (data) {
        ...
        const states = this.phone.meetChannelStates;
        let allUserIDState;
        try {
        ...
        const stateCount = allUserIDState.stateCount;
        const list = allUserIDState.userIDArray;
        ...
        const myself = _.find(list, (item) => item.userID == myUserID);
        if (!myself) {
